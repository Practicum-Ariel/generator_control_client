...
export default function TechVisit({techId, genId = "667a8c00e30b38c5dad90562"}) {
  ...
  useEffect(() => {
    apiReq({ url: `/visit/lastVisit/${genId}` })
      .then(r => {setLastVisit(r)})
  }, []);
  ...
  const timeToNextVisit = (dateA) => {
    ...
    const  didDayVisit = date.getUTCFullYear() === currentDate.getUTCFullYear() &&
                                date.getUTCMonth() === currentDate.getUTCMonth() &&
                                date.getUTCDate() === currentDate.getUTCDate()
    
    const nextVisit = {
      nextWeekVisit:  (nextWeekVisit > currentDate && Math.ceil(Math.abs(nextWeekVisit - date) / (1000 * 60 * 60 * 24))) || "passed",
      nextMonthVisit: (nextMonthVisit > currentDate && Math.ceil(Math.abs(nextMonthVisit - date) / (1000 * 60 * 60 * 24))) || "passed",
  ...
  return (
    <div className={styles.techVisit}>
      <div className={styles.alertNextVisit}>
        ...
        <span style={{ backgroundColor: DaysLeftToNextVisit.nextWeekVisit == "passed" ? 'red' : (DaysLeftToNextVisit.nextWeekVisit >= 4 ? 'green' : 'orange') }}>
          {`הזמן שנשאר לטיפול שבועי :${DaysLeftToNextVisit.nextWeekVisit}`}
        </span>
        <span style={{ backgroundColor: DaysLeftToNextVisit.nextMonthVisit == "passed" ? 'red' : (DaysLeftToNextVisit.nextMonthVisit >= 20 ? 'green' : 'orange') }}>
          {` הזמן שנשאר לטיפול חודשי :${DaysLeftToNextVisit.nextMonthVisit}`}
        </span>
        <span style={{ backgroundColor: DaysLeftToNextVisit.didDayVisit == "false" ? 'red' : 'grren'}}>
